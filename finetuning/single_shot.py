#!/usr/bin/env python3
"""
Train a full FullModel (backbone + head combined) on waveform data.
...
class FullModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = ConformerModel()
        self.head = HybridHead()

    def forward(self, x):
        embeddings = self.backbone(x, mode='just_embeddings')
        class_logits, reg_logits = self.head(embeddings)
...
WANDB_RUN_NAME = "full_training_200k"
...
def full_training(
    ...
    for epoch in range(epochs):
        ...
        if (epoch % 2) == 0:
            ...
            }, f"/sdf/home/c/carsmith/sdf_data/self_supervised_flash/full_training_200k/{epoch}.pth")
...
def main():
    ...
    train_loader, val_loader, test_loader = make_wf_dataloaders(
        DATA_PATH, batch_size=BATCH_SIZE, val_ratio=0.1, test_ratio=0.0, generator=g, norm=False
    )

    # Create full model and optimizer
    model = FullModel()
    ...
    # Run training
    results = full_training(model, train_loader, val_loader, optimizer, scheduler, device, epochs=EPOCHS, logger=logger)

    # Save results locally
    out_path = "full_training_results.pkl"
